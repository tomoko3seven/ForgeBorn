...
GTCEuStartupEvents.registry('gtceu:machine', event => {
	event.create('gtceu:kiln', 'primitive')
		...
		.pattern(definition => FactoryBlockPattern.start()
			.aisle('BBB', 'BBB', 'BBB', '#B#', '#B#')
			.aisle('BBB', 'BCB', 'B#B', 'B#B', 'B#B')
			.aisle('BBB', 'BDB', 'BBB', '#B#', '#B#')
			.where('B', Predicates.blocks('twigs:cobblestone_bricks'))
			.where('#', Predicates.any())
			.where('C', Predicates.blocks('minecraft:campfire'))
			.where('D', Predicates.controller(Predicates.blocks(definition.get())))
			.build())
		.workableCasingRenderer('twigs:block/cobblestone_bricks', 'gtceu:block/multiblock/primitive_blast_furnace', false)
		.editableUI(
			global.ui_builder({
				group: 'primitive',
				name: 'kiln',
