...
//Drones
const drone_type = ["harvest", "hunter", "miner"];
const drone_tier = ["basic", "advanced"];
for (const tier of drone_tier) {
        for (const type of drone_type) {
            event.create(`${tier}_drone_${type}`).maxStackSize(64).texture(`kubejs:item/drones/${type}_drone`);
        }
    }
...
event.create("air_bottle").displayName("Bottle of Air").tooltip('§8Fresh Air...')
  ...
  event.create("mana_chip").tooltip('§8Example of Technomancy')
        .tooltip([
            '§aULV Botanical Circuit',
        ]);
  event.create("basic_botanical_circuit").tooltip('§8Example of Technomancy')
        .tooltip([
  ...
  event.create("pure_mana_dust").maxStackSize(64)/*.texture(`kubejs:item/botania/manadiamond_dust`)*/;
