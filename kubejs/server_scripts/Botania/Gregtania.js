...
ServerEvents.recipes(event => {
    ...
    event.recipes.gtceu.assembler('kubejs:mana_chip')
        .itemInputs('2x gtceu:manasteel_plate', '2x gtceu:manasteel_foil', '2x gtceu:manasteel_bolt', '2x gtceu:fine_copper_wire')
    ...
    event.forEachRecipe({ type: "botania:mana_infusion" }, recipe => {
        ...
        event.recipes.gtceu.mana_infuser(`botania_conversion/${uniqueId}`)
            .itemInputs(inputItem)
            .inputFluids(`kubejs:dense_mana ${Math.floor(Math.max(1, manaCost / 10))}`)
            .itemOutputs(outputItem)
            .EUt(GTValues.VA[GTValues.MV])
            .duration(20)
    })
    ...
    event.recipes.gtceu.mana_infuser('manasteel_ingot_best')
        ...
        .duration(20)
        .EUt(GTValues.VA[GTValues.MV])
    event.recipes.botania.mana_infusion('botania:manasteel_ingot', 'gtceu:blue_alloy_ingot', 2700*10)
