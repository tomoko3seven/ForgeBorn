...
PlayerEvents.tick(event => {
  ...
  if (dim == OVERWORLD && pos.y <= -63 && data.standTicks >= STAND_TIME) {
    ...
    player.sendSystemMessage(Text.of("§6You feel strangely... Teleporation after 5 seconds!"));

    player.server.scheduleInTicks(TELEPORT_DELAY, () => {
      if (player && player.isAlive()) {
        clearArea(player); 
        player.teleportTo(UNDERGARDEN, pos.x + 0.5, CEILING_Y, pos.z + 0.5, player.yRot, player.xRot);
  ...
  // === Overworld ===
  if (dim == UNDERGARDEN && pos.y >= CEILING_Y && data.standTicks >= STAND_TIME) {
    data.teleporting = true;
    player.sendSystemMessage(Text.of("§6Returning to overworld after 5 seconds!"));

    player.server.scheduleInTicks(TELEPORT_DELAY, () => {
      if (player && player.isAlive()) {
        clearArea(player); 
        player.teleportTo(OVERWORLD, pos.x + 0.5, SAFE_OVERWORLD_Y, pos.z + 0.5, player.yRot, player.xRot);
        player.potionEffects.add("minecraft:slow_falling", 200, 0, false, true);
        clearArea(player); 
      }
      data.teleporting = false;
